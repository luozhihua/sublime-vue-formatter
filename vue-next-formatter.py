...
OUTPUT_VALID = b"*** VueNextFormatter output ***"

class VuenextformatterCommand(sublime_plugin.TextCommand):
  def run(self, edit):
...
class VuenextformatterEventListeners(sublime_plugin.EventListener):
  @staticmethod
  def on_pre_save(view):
    if PluginUtils.get_pref("format_on_save"):
      view.run_command("vuenextformatter")

class VuenextformatterSetPrettifyPrefsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    PluginUtils.open_config_rc(self.view.window())

class VuenextformatterSetPluginOptionsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    PluginUtils.open_sublime_settings(self.view.window())

class VuenextformatterSetKeyboardShortcutsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
...
class VuenextformatterSetNodePathCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    PluginUtils.open_sublime_settings(self.view.window())
