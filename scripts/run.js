...
var vue_beautify = require("./vue/formatter.js");
...
if (fs.existsSync(jsbeautifyrcPath = pluginFolder + path.sep + ".vueformatterrc")) {
...
pathsToLook.filter(Boolean).some(function(pathToLook) {
    if (fs.existsSync(jsbeautifyrcPath = path.join(pathToLook, ".vueformatterrc"))) {
        setOptions(jsbeautifyrcPath, options);
        return true;
    }
});
...
function setOptions(file, optionsStore) {
    ...
    for (var key in obj) {
        ...
        for (var pref in value) {
            ...
            // This avoids common accidents in .vueformatterrc json files.
